...
// is compiled) to determine the kinds of information knowable by [`bitwise`] and which must be
...
use syn::__private::{Span as Span2, TokenStream2};
...
///
/// [`bitwise`] does not support generically-parameterized items. This macro asserts that the
/// current item is not generic.
macro_rules! assert_no_generics {
    ($item_generics:expr $(,)?) => {
        if let Some(lt_token) = $item_generics.lt_token {
            fail!(lt_token.span, "generic parameters are not supported in this context");
        }
        // To be sure, if the `<` token is absent, there can't be any generic parameters (right)?
        debug_assert!($item_generics.params.is_empty());
    };
}

/// Enforces the `$expected_width` of an item.
///
/// [`bitwise`] accepts `size` and `width` attribute arguments which are intended to make
/// compile-time guarantees about the `Bitwise::WIDTH` of the generated item. In the simple case,
/// the width can be calculated exactly during macro evaluation and [`fail`] is called if it differs
/// from `$expected_width`. Otherwise, if the item is a struct containing custom types that
/// implement `Bitwise`, the exact width is indeterminate at macro evaluation time and so
/// enforcement of `$expected_width` is deferred to compile-time via an assertion in a const
/// context.
macro_rules! enforce_item_width {
    ($expected_width:expr, $item_span:expr, $item_width:expr, $const_ctx:expr $(,)?) => {
            ...
            match $item_width {
                Width::Lit(actual_width) => {
                    if expected_width != actual_width {
                        fail!($item_span, make_error_msg());
                    }
                }
                Width::Expr(ref actual_width) => {
                    ...
                    let panic_msg = make_error_msg();
                    $const_ctx.push(quote! {
                        if #expected_width != (#actual_width) {
...
impl ConstUsizeExpr {
    ...
    fn into_expr(self, span: Span2) -> syn::Expr {
        match self {
            Self::Lit(inner) => syn::ExprLit {
                attrs: Vec::new(),
                lit: syn::LitInt::new(&inner.to_string(), span).into(),
            }
            .into(),
    ...
    /// Like [`into_expr`](Self::into_expr) but the expression is wrapped in invisible delimiters
    /// that preserve order of operations.
    fn into_grouped_expr(self, span: Span2) -> syn::Expr {
        syn::ExprGroup {
            attrs: Vec::new(),
            // These should have the same span, I think. (?)
            group_token: syn::token::Group { span },
            expr: Box::new(self.into_expr(span)),
        }
        .into()
    }

    /// Applies a binary operation to two `ConstUsizeExpr`s and returns the result.
    ///
    /// `lhs` and `rhs` are wrapped in [`Cow`] to minimize cloning.
    fn apply_binop(span: Span2, lhs: Cow<Self>, rhs: Cow<Self>, op: syn::BinOp) -> Self {
        Self::Expr(syn::ExprBinary {
            attrs: Vec::new(),
            left: Box::new(lhs.into_owned().into_grouped_expr(span)),
            op,
            right: Box::new(rhs.into_owned().into_grouped_expr(span)),
        }
        .into())
    }
}
    ...
    (
        ...
        impl std::ops::$trait for ConstUsizeExpr {
            ...
            fn $fn(self, rhs: Self) -> Self::Output {
                if let (Self::Lit(lhs), Self::Lit(rhs)) = (self, rhs) {
                    Self::Lit(lhs $op rhs)
                } else {
                    // FIXME: fix span
                    let span = Span2::mixed_site();

                    Self::apply_binop(span, Cow::Owned(self), Cow::Owned(rhs), binop!(span, $op, $trait))
                }
            }
        }
...
fn determine_item_repr(
        ...
        item_attrs.iter().enumerate().find(|(_, attr)| attr.path().is_ident("repr"))
    {
        ...
        .round_up();
        if !repr.exists() {
            return Err(make_error(item_span, "this item cannot be represented by any existing integer primitive"));
        }
...
fn bitwise_on_enum(expected_width: Option<usize>, item: syn::ItemEnum) -> TokenStream {
    ...
    assert_no_generics!(item_generics);
